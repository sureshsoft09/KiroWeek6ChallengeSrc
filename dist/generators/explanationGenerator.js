...
exports.generateDimensionComparisons = generateDimensionComparisons;
exports.generateTradeoffAnalysis = generateTradeoffAnalysis;
exports.generateConditionalGuidance = generateConditionalGuidance;
exports.generateSummary = generateSummary;
exports.generateWeightingContext = generateWeightingContext;
exports.generatePairwiseExplanation = generatePairwiseExplanation;
 ...
 */
function generateDimensionComparisons(context) {
    ...
    types_1.ALL_DIMENSIONS.forEach(dimension => {
        ...
        if (Math.abs(delta) >= SIGNIFICANT_DELTA_THRESHOLD || Math.abs(rawDelta) >= 2) {
            const description = generateDimensionDescription(dimension, architecture1.architecture, architecture2.architecture, score1.rawScore, score2.rawScore, rawDelta, delta, weights[dimension], constraints);
            comparisons.push({
                dimension,
 ...
 */
function generateDimensionDescription(dimension, arch1, arch2, score1, score2, rawDelta, weightedDelta, weight, constraints) {
    ...
    const higherArchData = scoringMatrix_1.SCORING_MATRIX.architectures[higherArch];
    const lowerArchData = scoringMatrix_1.SCORING_MATRIX.architectures[lowerArch];
 ...
 */
function extractKeyPhrase(rationale) {
 ...
 */
function getConstraintContext(dimension, weight, constraints) {
 ...
 */
function generateTradeoffAnalysis(context) {
    const { architecture1, architecture2 } = context;
    const statements = [];
    // Get trade-offs from scoring matrix
    [architecture1, architecture2].forEach(archEval => {
        const archData = scoringMatrix_1.SCORING_MATRIX.architectures[archEval.architecture];
 ...
 */
function generateConditionalGuidance(context) {
    const { architecture1, architecture2, constraints } = context;
    const statements = [];
 ...
 */
function generateWeightingContext(context, dimensionComparisons) {
 ...
 */
function generatePairwiseExplanation(context) {
